...
export class InvoiceCultureAddpoComponent implements AfterViewInit {
  ...
  ngOnInit(): void {
    ...
    this.InvoiceCultureForm?.get('InvoiceCategory')?.valueChanges.subscribe((categoryId: number) => {
      if (categoryId == 1060) {
        // State Wise
        this.InvoiceCultureForm.get('State')?.disable();
        this.InvoiceCultureForm.get('State')?.setValue('');
  ...
  getSelectedInvoiceTypes() {
    return this.typeInvoiceList.filter(t =>
      this.InvoiceCultureForm.get(t.Paycode_Id.toString())?.value === true
    );
  }
