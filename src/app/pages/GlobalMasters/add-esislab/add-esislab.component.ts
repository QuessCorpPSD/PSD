...
export class AddESIslabComponent {
  ...
  patchEditSlab(row: any): void {
    ...
    this.effectiveDate = this.apiToInputDate(row.Effective_Date);
  ...
  private apiToinputDate(date: string | null): string | null {
    ...
    // Use template literal correctly and pad if needed
  ...
  private apiToInputDate(date: string | null): string | null {
    ...
    // date example: "01-01-2016 12:00:00 AM"
    // Split date and time
    const datePart = date.split(' ')[0]; // "01-01-2016"

    // Now split the datePart by '-'
    const [day, month, year] = datePart.split('-');

    // Return formatted as yyyy-MM-dd (input date format)
    return `${year}-${month.padStart(2, '0')}-${day.padStart(2, '0')}`;
  }
