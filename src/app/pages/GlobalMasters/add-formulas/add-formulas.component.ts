...
export class AddFormulasComponent {
  ...
  ngOnInit() {
    ...
    this.addformula.get('PayCategory')?.valueChanges.subscribe(categoryId => {
      if (categoryId) {
        this.onPayCategoryChange(categoryId);
      } else {
        // reset pay code if category cleared
        this.addformula.patchValue({ PayCode: '' });
        this.payCode = [];
      }
    });
  }
  onPayCategoryChange(categoryId: number) {
    // Reset PayCode
    this.addformula.patchValue({ PayCode: '' });

    // Load PayCode list based on category
    this.LoadpayCategory(categoryId);
  }
  ...
  ChangepayCategory(category: any) {
  ...
  ChangepayCode(event: Event) {
    ...
    this.addformula.patchValue({ PayCode: value });
  }
