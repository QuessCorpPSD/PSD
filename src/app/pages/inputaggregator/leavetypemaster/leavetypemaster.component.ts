...
export class LeavetypemasterComponent {
  ...
  isshowtable = false;
  leaveType: string = '';
  allLeaveTypes: any[] = [];   // 👈 store original data
  isActive: number = 1;
  userdetail: any;
  ...
  applyFilter(): void {
    ...
    this.leaveTypes = this.allLeaveTypes.filter(item =>
      item.leaveName?.toLowerCase().includes(filterValue) ||
      // item.leaveId?.toString().includes(filterValue) ||
  ...
  bindLeaveTypes(data: any[]): void {
    ...
    this.leaveTypes = [...this.allLeaveTypes];  // show all initially
  }
  // bindLeaveTypes(data: any[]): void {
